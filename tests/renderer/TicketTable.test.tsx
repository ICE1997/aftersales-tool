...
describe('TicketTable', () => {
  it('shows the first page (default 20 rows) and the total count', () => {
    render(<TicketTable tickets={mks(25)} onOpen={() => {}} />)
  ...
  it('shows the empty state when there are no tickets', () => {
    render(<TicketTable tickets={[]} onOpen={() => {}} />)
    expect(screen.getByText('暂无售后单')).toBeTruthy()
  })

  it('resets to page 1 when the tickets list changes', () => {
    const { rerender } = render(<TicketTable tickets={mks(25)} onOpen={() => {}} />)
    fireEvent.click(screen.getByText('下一页'))
    expect(screen.getByText('AS-21')).toBeTruthy()
    rerender(<TicketTable tickets={mks(25)} onOpen={() => {}} />)
  ...
  it('shows recipient name and region columns', () => {
    const onOpen = vi.fn()
    render(<TicketTable tickets={[mk({ aftersaleNo: 'AS-1', recipientName: '程玲', province: '江苏省', city: '苏州市', district: '虎丘区' })]} onOpen={onOpen} />)
  ...
  it('sorts by 申请时间 when its header is clicked', () => {
    ...
    render(<TicketTable tickets={tickets} onOpen={() => {}} />)
