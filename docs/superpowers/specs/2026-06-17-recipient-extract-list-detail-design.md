...
### 3.2 算法
1. **去噪**:删除所有 `[...]` 与 `【...】` 方括号及其内容(如 PDD 短码 `[2817]`)。保留圆括号(可能是地址的一部分)。按换行 + 连续空白切分为 tokens/lines,同时保留整体串用于地区匹配。
2. **手机号 + 分机号**:用 `1[3-9]\d{9}` 找第一个手机号;其后若紧跟 `转|分机|ext\.?|,|，|-|/` + `\d{1,6}`,把数字捕获为 `extension`。从工作文本中移除「手机号(+分机段)」子串。
   - 纯 11 位号(如 `19592642954`)→ phone 命中、extension 为 `''`。
   - 虚拟号(如 `1700000000转5678` / `170...,5678`)→ phone + extension。
3. **地区(最长匹配)**:在剩余文本中,
...
- **`extractContact`(纯函数,TDD,重点)**:
  - 用户样例(三行:`程玲[2817]` / `19592642954` / `江苏省苏州市虎丘区 龙湖时代100 8栋2207[2817]`)→ 断言 name=程玲、phone=19592642954、extension=''、province=江苏省、city=苏州市、district=虎丘区、addressDetail=`龙湖时代100 8栋2207`。
